...
@pytest.mark.parametrize("blueprint", ["tailwind", "bootstrap"])
def test_start_project(tmp_path, blueprint: str, runner: CommandRunner):
    runner.invoke("start-project", "dotfm", "--blueprint", blueprint)
    assert (tmp_path / "dotfm").exists()
    assert (tmp_path / "dotfm/dotfm/settings.py").exists()
