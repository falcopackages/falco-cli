...
import cappa
from falco.utils import FalcoConfig, get_pyproject_file
...
class InstallCrudUtils:
    ...
    def __call__(self, project_name: Annotated[str, cappa.Dep(get_project_name)]):
        ...
        if pyproject_path:
            write_falco_config(
                pyproject_path=pyproject_path, crud_utils=str(output_dir)
            )
    ...
    def install(self, project_name: str, falco_config: FalcoConfig) -> Path:
        output_dir = (
            self.output_dir
            or self.get_install_path(
                project_name=project_name, falco_config=falco_config
            )[0]
        )
    ...
    def get_install_path(
        cls, project_name: str, falco_config: FalcoConfig
    ) -> tuple[Path, bool]:
